                ...
                Moment(this.props.expiresAt).isAfter() ?
                  (
                    <div className="text-warning p-2">
                      <i className="fas fa-clock"></i> Selesai {Moment(this.props.expiresAt).locale('id').fromNow()}
                    </div>
                  ) : (
                    (Moment.duration(Moment(this.props.expiresAt).locale('id').diff()).humanize() !== 'Invalid date') && (
                      <div className="text-danger p-2">
                        Deadline {Moment.duration(Moment(this.props.expiresAt).diff()).locale('id').humanize()} yang lalu
                      </div>
                    )
                  )
              )
